...
async function search(q: string, cursor: string | null = null): Promise<Root | undefined> {
    ...
    if (!process.env.CENSYS_ID || !process.env.CENSYS_SECRET) {
        console.log(colors.yellow("CENSYS_ID or CENSYS_SECRET not found in .env file. Please add them to scrape CORS proxies."));
        return undefined;
    }

    const apiID = process.env.CENSYS_ID ?? "d973cf60-4ce4-4746-962b-815ddfdebf80",
        apiSecret = process.env.CENSYS_SECRET ?? "s6EUuA4Sfaajd8jDBJ17b4DaoPofjDe6";
